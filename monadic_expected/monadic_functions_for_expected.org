#+AUTHOR: Jeff Garland
#+TITLE: D2505R3 Monadic Functions for ~std::expected~
...
This question has been asked several times including in the LEWG 2022-05-03 meeting.  While less concise, the author prefers to keep the specification consistent with the rest of the c++23 library and especially ~std::optional~.  Limited access to compilers that implement deducing-this make it difficult to confirm correct behavior of a deducing-this based specification.

One potential design advantage of using deducing-this in the specification is improved SFINAE
...
- better compatiblity with future library facilities
...
Since the author is unaware of any existing practice with providing these functions and the purpose of the paper is consistency with monadic optional functions already voted into the working draft, the author will not pursue this option.  A fully thought out paper with existing practice would be required to make this change. Note that the author's experience with ~std::expected~ implementing json parsing is that ~expected~ would be extremely cumbersome for the task without these functions.  So shipping C++23 without these functions would be a major mistake for users.
...
#+BEGIN_SRC C++
       if (has_value()) { 
          ...
          return unexpected(invoke(std::forward<F>(f)(error())));
       }
#+END_SRC
    1 ~template <class F> constexpr auto transform_error(F&& f) &&;~
...
#+BEGIN_SRC C++
       if (has_value()) { 
          ...
          return unexpected(invoke(std::forward<F>(f)(std::move(error()))));
       }
#+END_SRC
...
* Acknowledgements
 ...
 - Thanks to Barry Revzin for design and wording feedback as well as sheparding in LEWG.
* Revision History
