#+AUTHOR: Jeff Garland
#+TITLE: D2505R0 Monadic Functions for ~std::expected~
...
For more examples see: https://godbolt.org/z/Tjo6YqoPn.
...
#+END_SRC
  ...
  2 ~template <class F> constexpr auto and_then(F&& f) const&&;~
    ...
    /Mandates:/ ~is_same_v<U, expected>~ is true.
...
#+BEGIN_SRC C++
       if (*this) { 
          ...
          return std::forward<F>(f)(invoke(error()));
       }
#+END_SRC
...
* References
  ...
  7. More complete examples https://godbolt.org/z/Tjo6YqoPn
