...
#+END_SRC
v   1 ~template <class F> constexpr auto transform(F&& f) &;~

   2 ~template <class F> constexpr auto transform(F&& f) const&;~

    Let U be ~remove_cvref_t<invoke_result_t<F, decltype(value())>>~.

    /Mandates:/ U is a `void` or non-array object type other than ~in_place_t~, ~unexpect_t~, or a specialization of ~unexpected~. If ~is_void_v<U>~ is ~false~, the declaration ~U u(invoke(std::forward<F>(f), value()));~ is well-formed.
...
#+END_SRC
   ...
   2 ~template <class F> constexpr auto transform(F&& f) const&;~
    ...
    /Mandates:/ U is a `void` or non-array object type other than ~in_place_t~, ~unexpect_t~, or a specialization of ~unexpected~. If ~is_void_v<U>~ is ~false~, the declaration ~U u(invoke(std::forward<F>(f)));~ is well-formed.
